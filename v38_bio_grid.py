...
if __name__ == "__main__":
    ...
    # Labels (Small)
    # Labels (Dynamic Vector Model)
    # Q1 (TL): -HRV (Stress), +TA (Rise) -> ACTIVE STRESS
    ax_grid.text(0.15, 0.90, "STRESS", transform=ax_grid.transAxes, fontsize=6, color='red', alpha=0.5)
    
    # Q3 (BL): -HRV (Stress), -TA (Fall) -> DEPLETION
    ax_grid.text(0.15, 0.05, "DEPLET", transform=ax_grid.transAxes, fontsize=6, color='gray', alpha=0.5)
    
    # Q2 (TR): +HRV (Recov), +TA (Rise) -> EXCITEMENT (Pos Arousal)
    ax_grid.text(0.65, 0.90, "EXCITE", transform=ax_grid.transAxes, fontsize=6, color='orange', alpha=0.5)
    
    # Q4 (BR): +HRV (Recov), -TA (Fall) -> RECOVERY (Active)
    ax_grid.text(0.65, 0.05, "RECOV", transform=ax_grid.transAxes, fontsize=6, color='green', alpha=0.5)
    ...
    TREND_MAP = {
        # Horizontal
        (2, 1): "Pivot to Flow",    (1, 2): "Loss of Control",
        (3, 4): "System Reboot",    (4, 3): "The Slump",
        # Vertical
        (1, 4): "Accomplishment",   (4, 1): "Powering Up",
        (2, 3): "The Crash",        (3, 2): "The Alarm",
        # Diagonal
        (2, 4): "Blowdown (Relief)",(4, 2): "The Jolt",
        (1, 3): "Fade Out",         (3, 1): "The Rally"
    }

    def update_grid(d_hrv, d_ta):
        global prev_quad
        
        # Scale Deltas to Grid (-6 to 6)
        # [FIX] Aggressive reduction for HRV (X-Axis) based on user feedback (L/R Clamping)
        SCALE_HRV = 0.15 # 40ms Delta -> 6.0 (Max)
        
        # [FIX] Increase Scaler for TA to Increase Sensitivity (User Request)
        # Prev: 25.0, User tried 5.0 (Low). New: 50.0 (High Sensitivity)
        SCALE_TA = 40.0  # 0.12 Delta -> 6.0 (Max)
        
        # X = Delta HRV (Right = Recover/Excite)
        z_x = max(-6.0, min(6.0, d_hrv * SCALE_HRV))
        
        # Y = Delta TA (Up = Arousal/Stress)
        ...
        if prev_quad is not None:
             if curr_q != prev_quad:
                  # Transition!
                  key = (prev_quad, curr_q)
                  if key in TREND_MAP:
                       trend_str = TREND_MAP[key]
                  else:
                       trend_str = "Transition..."
                  
                  # Update "Sticky" Trend? Or just Text?
                  txt_grid_trend.set_text(trend_str)
        else:
             # First Run
             pass
 ...
 # [FIX] Force clear for View Switch (Blit)
    ...
    def update(frame=0):
        ...
        if current_view == 'main':
            ...
            if bg_grid:   fig.canvas.restore_region(bg_grid) # [NEW]
            if bg_grid_txt: fig.canvas.restore_region(bg_grid_txt) # [NEW]
        ...
        global grid_hist_ta, grid_hist_hrv
        
        # Push current values (Looser Threshold)
        if latest_gsr_ta > 0.1: grid_hist_ta.append(latest_gsr_ta)
        if latest_hrv >= 0.0: grid_hist_hrv.append(latest_hrv) # [FIX] Allow 0.0 if sensor sends it (e.g. init)
        
        # Need at least ~1 sec history (50 frames) for stable delta
        if len(grid_hist_ta) > 50 and len(grid_hist_hrv) > 50:
             # Delta = Current (Avg of last 5) - Previous (Avg of 5 around -50)
             # Smoothing
             curr_ta = sum(list(grid_hist_ta)[-10:]) / 10.0
             prev_ta = sum(list(grid_hist_ta)[0:10]) / 10.0
             d_ta = curr_ta - prev_ta
             
             # HRV (slower update, same window logic useful?)
             # Yes, changes over 3s are relevant.
             # Wait, deque maxlen=150. Index 0 is -3.0s.
             # Let's use full window delta?
             # User said "Last 3 vs Previous 3".
             # Our loop is fast. Let's compare Now vs 2s Ago (Index -100).
             
             curr_hrv = sum(list(grid_hist_hrv)[-10:]) / 10.0
             prev_hrv = sum(list(grid_hist_hrv)[0:10]) / 10.0 # Oldest in buffer
             d_hrv = curr_hrv - prev_hrv
             
             # Call Update
             if 'update_grid' in globals():
                  update_grid(d_hrv, d_ta)
        # System Line
        sys_parts = []
