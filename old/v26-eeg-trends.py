...
TREND_WINDOW_SEC = 3.0
TREND_WINDOW_SAMPLES = int(FS * TREND_WINDOW_SEC) # 1250 samples for 5s smoothing 
...
# Control Defaults
FFT_WINDOW_SEC = 1.0  # Fixed Physics Constant (1Hz Resolution)
    ...
    try:
        ...
        if len(new_raw) > 0:
            ...
            for i, val in enumerate(filt_chunk):
                ...
                if headset_on_head and is_warmed_up and len(eeg_buffer) >= req_samples:
                     ...
                     for k, b_val in zip(['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma'], bands):
                         # [MODIFIED] No EMA smoothing. Raw FFT bands go to history.
                         # 5-second trend window handles the smoothing downstream.
            ...
            if is_recording and csv_writer:
                ...
                # Removed spike_detected_flag, always 0
                row = [ts, current_state, int(headset_on_head), 1 if event_detected else 0, 0, current_focus_score, current_calm_score] + bands + list(new_raw[:50])
...
def load_config():
    global current_mic_name, current_mic_gain, max_vol_scale, global_percent, triggers
    ...
    try:
        ...
        global coincidence_window, baseline_window_sec, triggers_enabled
...
if __name__ == "__main__":
    ...
    # --- MAIN DISPLAY: BAND POWER BARS + STATE INDICATORS ---
    # Replaces line graph with horizontal bars and LED indicators
    
    # === BAND POWER BARS (Top Section) ===
    ax_bars = reg_ax([0.05, 0.45, 0.70, 0.48], main_view_axes)
    ax_bars.set_xlim(0, 100)
    ax_bars.set_ylim(-0.5, 4.5)
    ax_bars.set_xlabel("Relative Power (%)", fontsize=10)
    ax_bars.set_title("EEG Band Power", fontsize=14, fontweight='bold')
    ax_bars.set_yticks([0, 1, 2, 3, 4])
    ax_bars.set_yticklabels(['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma'])
    ax_bars.grid(True, axis='x', alpha=0.3)
    
    # Create horizontal bars (will be updated in animation)
    bar_colors = ['blue', 'green', 'orange', 'red', 'purple']
    band_bars = ax_bars.barh([0, 1, 2, 3, 4], [0, 0, 0, 0, 0], color=bar_colors, alpha=0.7, height=0.6)
    
    # Value labels on bars
    bar_labels = [ax_bars.text(5, i, "0%", va='center', fontsize=11, fontweight='bold') for i in range(5)]
    
    # === STATE INDICATORS (Right Section) ===
    ax_states = reg_ax([0.78, 0.45, 0.17, 0.48], main_view_axes)
    ax_states.set_xlim(0, 1)
    ax_states.set_ylim(0, 4)
    ax_states.set_xticks([])
    ax_states.set_yticks([])
    ax_states.set_title("Mental States", fontsize=12, fontweight='bold')
    ax_states.set_facecolor('#f5f5f5')
    
    # LED circles and labels for each state
    state_leds = {}
    state_names = ['CALM', 'FOCUS', 'CONFLICT', 'RELIEF']
    state_colors_on = ['#44aa44', '#4444ff', '#ff4400', '#00ff44']
    state_colors_off = ['#cccccc', '#cccccc', '#cccccc', '#cccccc']
    
    for i, (name, col_on) in enumerate(zip(state_names, state_colors_on)):
        y_pos = 3.5 - i
        # LED circle
        circle = plt.Circle((0.2, y_pos), 0.12, color='#cccccc', ec='#666', linewidth=2)
        ax_states.add_patch(circle)
        state_leds[name] = circle
        # Label
        ax_states.text(0.4, y_pos, name, va='center', fontsize=11, fontweight='bold')
    
    # === SCORES DISPLAY (Below States) ===
    ax_scores = reg_ax([0.78, 0.25, 0.17, 0.18], main_view_axes)
    ax_scores.set_xlim(0, 1)
    ax_scores.set_ylim(0, 2)
    ax_scores.set_xticks([])
    ax_scores.set_yticks([])
    ax_scores.set_facecolor('#e8e8e8')
    txt_calm_score = ax_scores.text(0.5, 1.5, "Calm: --", ha='center', va='center', fontsize=14, fontweight='bold')
    txt_focus_score = ax_scores.text(0.5, 0.5, "Focus: --", ha='center', va='center', fontsize=14, fontweight='bold')
    ...
    # Legacy text objects (for compatibility)
    txt_score = ax_bars.text(0.50, 1.05, "", transform=ax_bars.transAxes, ha="center", visible=False)

    event_text = ax_bars.text(0.5, 0.5, "", transform=ax_bars.transAxes, visible=False)
    
    # State detection thresholds (OPTIMIZED from session analysis)
    # These match 4/4 Conflict and 2/3 Relief events from reference session
    STATE_THRESHOLDS = {
        'CALM_ALPHA_MIN': 15.0,         # Alpha >= 15% for CALM
        'FOCUS_RATIO_MIN': 1.2,         # (Beta+Gamma)/(Alpha+Theta) >= 1.2
        'CONFLICT_BETA_MIN': 17.0,      # Beta >= 17 (stricter)
        'CONFLICT_GAMMA_MIN': 17.0,     # Gamma >= 17 (stricter)
        'RELIEF_DELTA_MIN': 55.0,       # Delta >= 55 (stricter)
        'RELIEF_BETA_MAX': 9.0,         # Beta <= 9 (stricter)
        'RELIEF_GAMMA_MAX': 10.0,       # Gamma <= 10
    }
    
    # State hold timers
    state_hold_until = {'CONFLICT': 0, 'RELIEF': 0}
    
    ui_refs = {}

    # [MODIFIED] Removed legacy trigger UI (checkboxes/buttons) as requested
    # We now always track all bands for the trend display.
    ...
    # [REMOVED] Trigger Settings Panel
    ...
    def apply_theme(theme_name):
        ...
        # Apply to Main Bar Graph
        try:
            ax_bars.set_facecolor(c_ax)
            ax_bars.tick_params(colors=c_fg)
            ax_bars.xaxis.label.set_color(c_fg); ax_bars.yaxis.label.set_color(c_fg)
            ax_bars.title.set_color(c_fg)
            for spine in ax_bars.spines.values(): spine.set_color(c_fg)
        except: pass
        
        # Apply to State Indicators panel
        try:
            ax_states.set_facecolor(c_panel)
            ax_states.title.set_color(c_fg)
        except: pass
        ...
        try:
            ...
            txt_focus_score.set_color(c_fg)
        except: pass
        ...
        # Apply to Settings panels (if they exist)
        try:
    ...
    # [REMOVED] Trigger Settings UI elements as requested


    # [REMOVED] Signal Processing controls (Hardcoded)
    ...
    def update(frame):
        ...
        if should_update_graph:
            # Get current band values (Smoothed Trend)
            band_values = []
            band_keys = ['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma']
            for k in band_keys:
                if len(bands_history[k]) > 0:
                    # [MODIFIED] Use Mean of last N samples (Trend) for valid display
                    # This replaces rapid flickering with "GSR-style" slow movements
                    dlist = list(bands_history[k])
                    samples_to_use = min(len(dlist), TREND_WINDOW_SAMPLES)
                    window = dlist[-samples_to_use:]
                    band_values.append(np.mean(window))
                else:
                    band_values.append(0)
            
            # === UPDATE BAND POWER BARS ===
            for i, (bar, val, label) in enumerate(zip(band_bars, band_values, bar_labels)):
                bar.set_width(min(val, 100))
                label.set_text(f"{val:.1f}%")
                label.set_x(min(val, 95) + 2)
            
            # === UPDATE SCORES ===
            txt_calm_score.set_text(f"Calm: {current_calm_score}")
            txt_focus_score.set_text(f"Focus: {current_focus_score}")
            
            # === STATE DETECTION (EEG-only) ===
            delta, theta, alpha, beta, gamma = band_values
            now = time.time()
            
            # CALM: Alpha > threshold
            is_calm = alpha >= STATE_THRESHOLDS['CALM_ALPHA_MIN']
            state_leds['CALM'].set_facecolor('#44aa44' if is_calm else '#cccccc')
            
            # FOCUS: (Beta + Gamma) / (Alpha + Theta) > ratio
            try:
                focus_ratio = (beta + gamma) / max(0.1, alpha + theta)
                is_focus = focus_ratio >= STATE_THRESHOLDS['FOCUS_RATIO_MIN']
            except:
                is_focus = False
            state_leds['FOCUS'].set_facecolor('#4444ff' if is_focus else '#cccccc')
            
            # CONFLICT: Beta >= threshold AND Gamma >= threshold
            is_conflict_triggered = (beta >= STATE_THRESHOLDS['CONFLICT_BETA_MIN'] and 
                                    gamma >= STATE_THRESHOLDS['CONFLICT_GAMMA_MIN'])
            if is_conflict_triggered:
                state_hold_until['CONFLICT'] = now + 2.0  # Hold for 2s
            is_conflict = now < state_hold_until['CONFLICT']
            state_leds['CONFLICT'].set_facecolor('#ff4400' if is_conflict else '#cccccc')
            
            # RELIEF: Delta >= threshold AND Beta/Gamma low
            is_relief_triggered = (delta >= STATE_THRESHOLDS['RELIEF_DELTA_MIN'] and 
                                  beta <= STATE_THRESHOLDS['RELIEF_BETA_MAX'] and
                                  gamma <= STATE_THRESHOLDS['RELIEF_GAMMA_MAX'])
            if is_relief_triggered:
                state_hold_until['RELIEF'] = now + 3.0  # Hold for 3s
            is_relief = now < state_hold_until['RELIEF']
            state_leds['RELIEF'].set_facecolor('#00ff44' if is_relief else '#cccccc')
        ...
        if current_view == 'main':
             # Main View Artists (bar-based display)
             artists.extend([rec_text, txt_conn, txt_wear, txt_batt])
             artists.extend(list(band_bars))
             artists.extend(bar_labels)
             artists.extend(list(state_leds.values()))
             artists.extend([txt_calm_score, txt_focus_score])
