...
class SessionReplayer:
    def __init__(self, master):
        self.master = master
        self.master.title("Emergent Knowledge Brainwave Session Replayer v10")
    ...
    def load_session(self):
        path = filedialog.askopenfilename(initialdir="Session_Data",
                                          filetypes=[("CSV Files", "*_detailed_*.csv"), ("All CSV", "*.csv")])
        if not path: return
        ...
        try:
            ...
            notes_path = os.path.join(dirname, f"brainwave_notes_{ts}.txt")
