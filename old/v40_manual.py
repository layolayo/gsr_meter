...
if __name__ == "__main__":
    ...
    # Labels (Small)
    # [NEW] Axis Labels
    ax_grid.text(0.5, 0.95, "NO CONFRONT", transform=ax_grid.transAxes, ha='center', fontsize=5, color='gray')
    ax_grid.text(0.5, 0.02, "CONFRONT", transform=ax_grid.transAxes, ha='center', fontsize=5, color='gray')
    ax_grid.text(0.03, 0.5, "STRESS", transform=ax_grid.transAxes, va='center', rotation=90, fontsize=5, color='red')
    ax_grid.text(0.95, 0.5, "RECOVERY", transform=ax_grid.transAxes, va='center', rotation=-90, fontsize=5, color='green')

    # Q1 (TR): +HRV (Recov), +TA (Rise/Mass) -> DISCONNECT (Safe but Avoidant)
    ax_grid.text(0.60, 0.87, "DISCONNECT", transform=ax_grid.transAxes, fontsize=6, color='orange', alpha=0.5)
    
    # Q2 (TL): -HRV (Stress), +TA (Rise/Mass) -> RESISTANCE (Unsafe Avoidance)
    ax_grid.text(0.13, 0.87, "RESISTANCE", transform=ax_grid.transAxes, fontsize=6, color='red', alpha=0.5)
    
    # Q3 (BL): -HRV (Stress), -TA (Fall/Action) -> OVERWHELM (Unsafe Confront)
    ax_grid.text(0.10, 0.07, "OVERWHELM", transform=ax_grid.transAxes, fontsize=6, color='gray', alpha=0.5)

    # Q4 (BR): +HRV (Recov), -TA (Fall/Action) -> INTEGRATION (Safe Confront)
    ax_grid.text(0.60, 0.07, "INTEGRATION", transform=ax_grid.transAxes, fontsize=6, color='green', alpha=0.5)
    ...
    grid_arrow = ax_grid.annotate("", xy=(0,0), xytext=(0,0), arrowprops=dict(arrowstyle="->", color='black', lw=2), zorder=5)
    ...
    def update_grid(d_hrv, d_ta, d_hr=0.0):
        ...
        # [FIX] Aggressive reduction for HRV (X-Axis) based on user feedback (L/R Clamping)
        SCALE_HRV = 0.10 # 40ms Delta -> 6.0 (Max)
        
        # [FIX] Increase Scaler for TA to Increase Sensitivity (User Request)
        # Prev: 25.0, User tried 5.0 (Low). New: 50.0 (High Sensitivity)
        SCALE_TA = 15.0  # 0.12 Delta -> 6.0 (Max)
        
        # X = Delta HRV (Right = Recover/Excite)
        z_x = max(-6.0, min(6.0, d_hrv * SCALE_HRV))
        
        # Y = Delta TA (Up = Arousal/Stress/Rise)
        # [FIX] Reverted to Direct Mapping:
        # +TA (Increase/Rest/Mass) -> Rise (Up, Y+)
        # -TA (Decrease/Action/Blowdown) -> Fall (Down, Y-)
        ...
        # [NEW] HR Delta Color Logic
        # Rising HR (+d_hr) -> RED
        # Lowering HR (-d_hr) -> GREEN
        # Stable -> WHITE
        
        import matplotlib.colors as mcolors
        import numpy as np
        
        # Clamp Delta to +/- 5 BPM over 3s window (Adjust sensitivity as needed)
        # Sensitivity: 5 BPM change is "Max Color"
        hr_sens = 5.0
        n_hr = max(-1.0, min(1.0, d_hr / hr_sens))
        
        # Interpolate
        # If n_hr > 0: White -> Red
        # If n_hr < 0: White -> Green
        ...
        if n_hr > 0:
             # Positive (Rise) -> Red
             final_c = (1.0 - n_hr) * c_white + n_hr * c_red
        else:
             # Negative (Fall) -> Green
             f = abs(n_hr)
             final_c = (1.0 - f) * c_white + f * c_green
             
        grid_dot.set_color(final_c)
        ...
        elif curr_q == 2: c_text = 'red'
        elif curr_q == 3: c_text = 'gray'
        elif curr_q == 4: c_text = 'green'
        ...
        if prev_quad is not None and prev_quad != curr_q and curr_q != 0:
             ...
             if pair in TREND_MAP:
                  ...
                  txt_grid_trend.set_color('blue')
                  # Hold trend text?
             prev_quad = curr_q
        elif curr_q == 0:
             # In center
             txt_grid_trend.set_text("--")
             txt_grid_trend.set_color('#444')
    ...
    # New Coordinates: Right Column (0.835), y=0.23, w=0.13, h=0.04
    r_man = [0.835, 0.23, 0.13, 0.04]
