...
if __name__ == "__main__":
    ...
    def update(frame):
        ...
        s_str = f"Name: {dev_static.get('name','?')} | SN: {dev_static.get('serial','?')} | HW: {dev_static.get('hw','?')} | FW: {dev_static.get('fw','?')} | Manuf: {dev_static.get('manuf','?')} | Batt: {device_battery_level} | Mic: {audio_handler.current_mic_name}"
                    ...
                    if calib_step in [1, 2, 3]:
                        if calib_phase == 0: # WAIT FOR DROP
                             ...
                             if time.time() - calib_start_time > 1.5:
                                 ...
                                 measured_win = max(0.05, min(50.0, measured_win))
