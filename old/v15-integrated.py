    ...
    def start(self, filename=None):
        with self.lock:
            ...
            if not filename:
                ...
                filename = f"Session_Data/Integrated_Session_{ts_str}.csv"
            ...
            self.recorded_samples = 0 # Reset counter
            print(f"[REC] Started: {filename}")

    def log_packet(self, eeg_conn, eeg_on, focus, calm, bands, raw, calculated_timestamp=None):
        with self.lock:
        ...
        if len(extracted) > 0:
            ...
            for i, val in enumerate(filt_chunk):
                ...
                if global_session_logger and global_session_logger.is_recording:
                    # [FIX] Exact Timestamp Calculation (250Hz stepping)
                    # This eliminates "jumps" caused by wall-clock variations
                    sample_offset = (samples_since_contact if headset_on_head else 0) 
                    # Better: use start time of recording
                    if global_session_logger.start_time > 0:
                         exact_time = global_session_logger.start_time + (global_session_logger.recorded_samples / FS)
                         global_session_logger.log_packet(
                            True, headset_on_head, 
                            current_focus_score, current_calm_score, 
                            bands, extracted[i],
                            calculated_timestamp=exact_time
                         )
    ...
    def run(self):
        try:
            ...
            while self.running:
                ...
                time.sleep(0.002) # [FIX] Yield GIL to allow BLE thread to run smoothly
    ...
    def load_gsr_config(self):
        ...
        try:
            with open(GSR_CONFIG_FILE, 'r') as f:
                ...
                print(f"[CONFIG] Loaded: Booster={self.booster_level}, Graph={self.graph_mode}")
        except Exception as e:
