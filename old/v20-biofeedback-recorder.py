...
class SessionLogger:
    def __init__(self):
        ...
        self.start_time = 0
        self.last_log_time = 0.0 # [NEW] Track last timestamp for monotonicity
    ...
    def start(self, filename=None, start_time=None):
        with self.lock:
            ...
            self.last_log_time = 0.0 # Reset
            self.recorded_samples = 0
            print(f"[REC] Started: {filename} (BaseTime: {self.start_time:.4f})")

    def _get_monotonic_time(self, requested_time):
        """Ensures time never moves backwards in the log."""
        elapsed = requested_time - self.start_time
        
        # [FIX] Strict Monotonicity
        # If new sample is 'older' than last written sample, clamp it to last + epsilon
        if elapsed < self.last_log_time:
             # Force it to be at least monotonic
             elapsed = self.last_log_time + 0.000001
    ...
    def log_packet(self, eeg_conn, eeg_on, focus, calm, bands, raw, calculated_timestamp=None, override_gsr=None):
        with self.lock:
            ...
            if calculated_timestamp:
                target_time = calculated_timestamp
            else:
                target_time = time.time()
                
            elapsed = self._get_monotonic_time(target_time)
            now = self.start_time + elapsed # Reconstruct absolute time if needed
