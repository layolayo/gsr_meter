...
from modules.gsr_patterns import GSRPatterns # [NEW] Pattern Rec
...
CALIB_TARGET_RATIO = 1.0 / 2.0 # [REQ] Target Drop % of screen (1/3rd)
...
def get_effective_window():
    ...
    # [REQ] Motion Safety: If Locked, FREEZE sensitivity at last known good value.
...
# [REQ] 60Hz * 10s = 600 samples (60Hz Plotting) -> Increased to 800 to ensure full coverage
HISTORY_LEN = 800
...
class GSRReader(threading.Thread):
    def __init__(self):
        ...
        self.last_flush_time = 0 # [FIX] Track flush timing
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    if is_recording:
                         try:
                             ...
                             if writer_gsr:
                                  ...
                                  writer_gsr.writerow([ts_now, f"{self.current_ta:.5f}", f"{GSR_CENTER_VAL:.3f}", f"{1.0/win:.3f}", f"{win:.3f}", is_motion, f"{CALIB_PIVOT_TA:.3f}", active_boost_level, note, current_pattern])
                             
                             # Log Trend (Sampled at GSR rate)
                             if writer_trend and is_connected:
                    ...
                    # [FIX] Periodic Flush to prevent IO Stalls (Every 1.0s)
                    if is_recording and (time.time() - self.last_flush_time) > 1.0:
                        try:
                            if f_gsr: f_gsr.flush()
                            if f_trend: f_trend.flush()
                    ...
                    try:
                         ...
                         # [FIX] Removed redundant manual popping - deque has maxlen
                         ...
                         # [OPT] Batch Processing: Ingest all samples first, then FFT ONCE.
                         batch_count = 0
                         ...
                         # Only calculate bands if we processed new data and have enough history
                         if batch_count > 0 and len(self.eeg_buffer) >= req_samples:
                             # [FIX] Thread-Safe Snapshot already local
                             clean_data = list(self.eeg_buffer)[-req_samples:] 
                             ...
                             for i, k in enumerate(['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma']):
                                 ...
                                 # [OPT] Push same value N times to preserve timebase without N FFTs
                ...
                time.sleep(0.0166) # [FIX] ~60Hz Pacing (was 0.015/66Hz) 
...
if __name__ == "__main__":
    ...
    def update_gsr_center(val, force_pivot=False):
    ...
    def on_key(event):
        ...
        if event.key == 'up':
            ...
            if latest_gsr_ta > 0.1:
                ...
                log_msg(f"User Reset GSR Center to {latest_gsr_ta:.3f}")
                active_boost_level = booster_level
                update_gsr_center(latest_gsr_ta)
    ...
    def start_actual_recording():
        ...
        try:
            ...
            #  User said: "then after that, TA Counter is initiated (if it was already started 0- then reset and restart)"
             ...
             writer_gsr.writerow(["Timestamp", "TA", "TA SET", "Sensitivity", "Window_Size", "Motion", "Pivot", "Boost", "Notes", "Pattern"])
    ...
    def toggle_rec(e):
        ...
        if not is_recording:
             ...
             if calib_mode:
                 ...
                 # [FIX] Ensure Audio Thread stops if we abort here
    ...
    def update(frame):
        global motion_lock_expiry, motion_prev_ta, motion_prev_time # [FIX] Added here
        ...
        if current_state == "EEG: STREAMING" or current_view == 'main':
             ...
             if latest_gsr_ta > 0.1:
                  ...
                  if 'txt_pattern' in ui_refs:
                       ...
                       # [FIX] Force explicit draw request if blitting implies it? 
                       # No, appending to 'artists' at end of loop is enough.
        ...
        if current_view == 'main':
                ...
                try:
                    ...
                    if calib_step in [1, 2, 3]:
                        if calib_phase == 0: # WAIT FOR DROP
                             ...
                             # [FIX] Thread-Safety: list(data) snapshot
                             history = list(bands_history['GSR'])[-60:]
                             is_steady = False
                             rng = 1.0
                             if len(history) > 30:
                             ...
                             if time.time() - calib_start_time > 1.5:
                                 ...
                                 # Rules: 
                                 # [REQ] Target: Drop = CALIB_TARGET_RATIO of Window
                                 
                                 # Hysteresis Bounds (+/- 25% approx)
                                 R_UPPER = CALIB_TARGET_RATIO * 1.3
                                 R_LOWER = CALIB_TARGET_RATIO * 0.7
                                 
                                 if current_ratio > R_UPPER or current_ratio < R_LOWER:
                                     target_win = total_drop / CALIB_TARGET_RATIO
                        ...
                        if calib_phase == 0:
                            ...
                            if is_stable and time_in_phase > 4.0:
                                ...
                                if False: # Disabled Check
                                    ...
                                    # Target: Drop = CALIB_TARGET_RATIO of Window
                                    target_win = total_drop / CALIB_TARGET_RATIO
                                    target_win = max(0.05, min(50.0, target_win))
                                    BASE_SENSITIVITY = target_win
                                    
                                    update_gsr_center(latest_gsr_ta)
                                    active_event_label = f"BREATH_OK | RESET SENSITIVITY"
             ...
             if 'txt_pattern' in ui_refs: artists.append(ui_refs['txt_pattern']) # [FIX] Add to artists 
