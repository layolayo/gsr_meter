...
# [NEW] Real-time Biofeedback Detection State
# Based on analysis: Conflict = flicker (TA stays high), Relief = genuine drop
biofeedback_state = {
    'conflict': False,
    'relief': False,
    'conflict_hold_until': 0.0,  # Time to hold indicator on
    'relief_hold_until': 0.0,
}

# [NEW] GSR History for Trend Detection
gsr_history = collections.deque(maxlen=100)     # 10s at 10Hz
gsr_trend_rising = False
gsr_rise_start_ta = 0.0
gsr_peak_ta = 0.0
gsr_last_drop_time = 0.0

# [NEW] EEG-GSR Combined Thresholds (from analysis.py)
# GSR Detection
GSR_NOISE_FLOOR = 0.0017         # Typical noise σ
GSR_FLICKER_THRESH = GSR_NOISE_FLOOR * 1.5   # For Conflict detection
GSR_RELIEF_THRESH = GSR_NOISE_FLOOR * 3.0    # For Relief detection  
GSR_MIN_RISE_SLOPE = 0.0003      # Minimum slope for "rising" trend
GSR_MIN_RISE_DURATION = 30       # 3 seconds at 10Hz
GSR_RECOVERY_CONFLICT = 0.3      # <30% recovery = Conflict
GSR_RECOVERY_RELIEF = 1.7        # 170%+ recovery = Relief

# EEG Thresholds (from analysis)
EEG_RELIEF_DELTA_MIN = 50.0      # Delta should be elevated during Relief
EEG_RELIEF_BETA_MAX = 10.5       # Beta should be low during Relief
EEG_RELIEF_GAMMA_MAX = 10.0      # Gamma should be low during Relief
EEG_CONFLICT_BETA_MIN = 13.0     # Beta elevated during Conflict
EEG_CONFLICT_GAMMA_MIN = 13.0    # Gamma elevated during Conflict

# [LEGACY] Surge flags (kept for backwards compatibility)
surge_flags = {'delta': False, 'theta': False, 'gamma': False}
# [NEW] Running Band History for Surge Detection
running_bands = {k: collections.deque(maxlen=50) for k in ['Delta', 'Theta', 'Gamma']} # 50 samples @ 5Hz updates = 10s
...
MOVEMENT_THRESHOLD = 0.20 # [FIX] Increased from 0.05 to reduce false positives during rapid physiological drops
...
class SessionLogger:
    def __init__(self):
        ...
        self.file_gsr = None
        self.file_eeg = None
        self.file_sys = None # [NEW] System Log
        self.writer_gsr = None
        self.writer_eeg = None
        self.is_recording = False
        self.start_time = 0
        self.last_log_time_gsr = 0.0 # Strict Monotonicity for GSR
        self.last_log_time_eeg = 0.0 # Strict Monotonicity for EEG
        self.last_log_time_gsr = 0.0 # Strict Monotonicity for GSR
        self.last_log_time_eeg = 0.0 # Strict Monotonicity for EEG
        self.lock_gsr = threading.Lock() # [FIX] Independent Lock for GSR
        self.lock_eeg = threading.Lock() # [FIX] Independent Lock for EEG

        # Ensure root directory exists
        if not os.path.exists("Session_Data"):
            os.makedirs("Session_Data")

    def start(self, session_name=None, start_time=None):
        # Use both locks to prevent partial starts
        with self.lock_gsr:
          with self.lock_eeg:
            if self.is_recording: return
            
            # Create Session Folder: Session_Data/Session_YYYY.../
            ts_str = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            if not session_name:
                session_name = f"Session_{ts_str}"
            
            # [FIX] Set start_time FIRST so logs are correct
            if start_time: self.start_time = start_time
            else: self.start_time = time.time()
            
            self.folder_path = os.path.join("Session_Data", session_name)
            os.makedirs(self.folder_path, exist_ok=True)
            
            # Open Files
            fn_gsr = os.path.join(self.folder_path, "gsr.csv")
            fn_eeg = os.path.join(self.folder_path, "eeg.csv")
            
            self.file_gsr = open(fn_gsr, mode='w', newline='')
            self.file_eeg = open(fn_eeg, mode='w', newline='')
            
            self.writer_gsr = csv.writer(self.file_gsr)
            self.writer_eeg = csv.writer(self.file_eeg)
            
            # 2. System Log (Text)
            sys_path = os.path.join(self.folder_path, "system.log")
            self.file_sys = open(sys_path, 'w', buffering=1) # Line buffered
            self.log_system(f"Session Started: {self.start_time}")
            
            # 3. CSV Headers
            self.writer_gsr.writerow([
                "Timestamp", "Elapsed_Sec", 
                "GSR_TA", "GSR_SetPoint", "GSR_Sens", "GSR_Motion"
            ])
            
            self.writer_eeg.writerow([
                "Timestamp", "Elapsed_Sec",
            ...
            self.last_log_time_gsr = 0.0
            self.last_log_time_eeg = 0.0
            self.recorded_samples = 0
            self.next_heartbeat = 250 # [NEW] Tracking for heartbeat
            print(f"[REC] Started in folder: {self.folder_path}")

    def _get_monotonic_time(self, requested_time, stream_type='gsr'):
        """Ensures time never moves backwards per stream."""
        elapsed = requested_time - self.start_time
        
        last = self.last_log_time_gsr if stream_type == 'gsr' else self.last_log_time_eeg
        
        if elapsed < last:
             elapsed = last + 0.000001
             
        if stream_type == 'gsr': self.last_log_time_gsr = elapsed
        else: self.last_log_time_eeg = elapsed
        
        return elapsed

    def log_eeg_packet(self, eeg_conn, eeg_on, focus, calm, bands, raw, calculated_timestamp=None):
        """Logged by BLE Thread"""
        with self.lock_eeg:
            if not self.is_recording: return
            
            target_time = calculated_timestamp if calculated_timestamp else time.time()
            elapsed = self._get_monotonic_time(target_time, 'eeg')
            ...
            try:
                ...
                # [DEBUG] Force flush and log heartbeat every ~1s (250 samples at 250Hz)
                if self.recorded_samples % 250 == 0:
                     self.file_eeg.flush()
                     # self.log_system(f"EEG Heartbeat: {self.recorded_samples} samples") 
            except Exception as e: 
                self.log_system(f"[REC] EEG Write Error: {e}")

    # [FIX] Batch write for performance (Match v10)
    def log_eeg_chunk(self, rows):
        # [TRACE] 3. Logger Enter

        # if self.file_sys: self.log_system("TRACE: Logger Enter (Acquiring Lock)")
        
        if self.file_eeg and self.writer_eeg:
            try:
                self.writer_eeg.writerows(rows)
                self.recorded_samples += len(rows)
                
                # [DEBUG] Heartbeat (Keep strict flush for diagnosis, but less freq?)
                # [FIX] Use inequality to ensure we don't miss the trigger with batch sizes != 1
                if self.recorded_samples > self.next_heartbeat: 
                     self.next_heartbeat += 250
                     self.file_eeg.flush()
                     # self.log_system(f"EEG Heartbeat: {self.recorded_samples} samples")
            except Exception as e:
                self.log_system(f"[REC] EEG Batch Error: {e}")
            

            


    def log_gsr_sample(self, ta, center, sens, is_motion):
        """Logged by Main/Serial Thread"""
        with self.lock_gsr:
            if not self.is_recording: return
            
            target_time = time.time()
            elapsed = self._get_monotonic_time(target_time, 'gsr')
            now = self.start_time + elapsed
            
            try:
                self.writer_gsr.writerow([
                    f"{now:.4f}", f"{elapsed:.4f}",
                    f"{ta:.5f}", f"{center:.5f}", f"{sens:.3f}", int(is_motion)
                ])
                self.recorded_samples += 1
            except Exception as e: print(f"[REC] GSR Write Error: {e}")

    def log_system(self, msg):
        """Log system events to text file"""
        if self.file_sys:
             try:
                 ts = datetime.datetime.now().strftime("%H:%M:%S.%f")[:-3]
                 self.file_sys.write(f"[{ts}] {msg}\n")
                 self.file_sys.flush() # [FIX] Flush immediately for debugging crashes
             except: pass

    def stop(self):
        # [FIX] Close both streams safely
        with self.lock_gsr:
             if self.file_gsr: self.file_gsr.close(); self.file_gsr = None
        
        with self.lock_eeg:
             if self.file_eeg: self.file_eeg.close(); self.file_eeg = None
             
        if self.file_sys: self.file_sys.close(); self.file_sys = None
        self.is_recording = False
        print("[REC] Saved Dual Streams.")
...
def log_msg(msg):
    if global_session_logger:
        global_session_logger.log_system(msg)
...
def notification_handler(sender, data):
    ...
    global zipper
    global last_on_signal_time # [FIX] Explicit global to ensure logging sees update
    
    try:
        # [TRACE] 1. Handler Entry
        # [TRACE] 1. Handler Entry
        # if 'global_session_logger' in globals() and global_session_logger:
        #    # [DEBUG] Log full packet to see why parsing fails
        #    hex_data = data.hex()
        #    global_session_logger.log_system(f"TRACE: Handler Enter Len={len(data)} Data={hex_data[:60]}...")
        
        if not data.startswith(b'CMSN'): return
        
        # [DEBUG] Prove Bleak is delivering packets (Log 1 per sec)
        # if samples_since_contact % 250 == 0:
             # Using log_msg directly to system.log since we have the redirect
             # if 'global_session_logger' in globals() and global_session_logger:
             #      global_session_logger.log_system(f"PKT RX: {data[:4]} (Alive)")
             
        # [FIX] Only update Watchdog time if packet is VALID protocol
        ...
        def parse_payload(payload):
            ...
            while idx < len(payload):
                try: # Bounds Check
                    ...
                    if key in [0x12, 0x32]:
                    ...
                    elif key == 0x08: # [FIX] Sequence Number is VarInt!
                        # Consume VarInt loop to prevent desync at 128 (0x80)
                        while idx < len(payload):
                        ...
                        if idx < len(payload):
                            ...
                            if val == 1:
                                ...
                                # [FIX] Reduced Debounce to 1.5s (As requested)
                                if (now - last_on_signal_time) > 1.5:
                                    if headset_on_head: 
                                        headset_on_head = False; print("Headset: OFF HEAD (Signal Lost)") 
        ...
        if len(extracted) > 0:
            ...
            # [FIX] Batch Logging (Match v10 Performance)
            batch_rows = []
            ...
            for i, val in enumerate(filt_chunk):
                ...
                if headset_on_head and len(eeg_buffer) >= FS:
                     ...
                     # [OPTIMIZATION] Calculate FFT every 5 samples (50Hz updates) instead of every 1 (250Hz)
                     # The user suggested "every 10", but 5 is a safe middle ground for responsiveness.
                     # We reuse 'last_computed_bands' for the intervening samples.
                     if i % 5 == 0:
                         recent = list(itertools.islice(eeg_buffer, len(eeg_buffer) - FS, len(eeg_buffer)))
                         # Use 1s window (FS samples) for 1Hz frequency resolution. 
                         # (50 samples would be too short for Alpha/Theta separation)
                         ...
                         if new_bands != [0]*5:
                             ...
                             # =========================================================
                             # [NEW] Combined GSR+EEG Biofeedback Detection
                             # Based on analysis.py findings:
                             # - CONFLICT: Rising TA + flicker + high Beta/Gamma
                             # - RELIEF: Strong drop + high Delta + low Beta/Gamma
                             # =========================================================
                             
                             # Update Running History (approx 10s window at 5Hz updates)
                             running_bands['Delta'].append(bands[0])
                             running_bands['Theta'].append(bands[1])
                             running_bands['Gamma'].append(bands[4])
                             
                             # Get current GSR state
                             global gsr_history, gsr_trend_rising, gsr_rise_start_ta, gsr_peak_ta
                             global gsr_last_drop_time, biofeedback_state
                             
                             current_ta = latest_gsr_ta
                             current_time = time.time()
                             
                             # Add to GSR history
                             gsr_history.append(current_ta)
                             
                             if len(gsr_history) >= 30:  # Need at least 3s of data
                                 # Calculate trend (simple slope over last 30 samples = 3s)
                                 recent_gsr = list(gsr_history)[-30:]
                                 gsr_slope = (recent_gsr[-1] - recent_gsr[0]) / 30.0
                                 
                                 # Calculate instantaneous change
                                 if len(gsr_history) >= 2:
                                     d_ta = list(gsr_history)[-1] - list(gsr_history)[-2]
                                 else:
                                     d_ta = 0.0
                                 
                                 # Track rising phase
                                 if gsr_slope > GSR_MIN_RISE_SLOPE:
                                     if not gsr_trend_rising:
                                         gsr_trend_rising = True
                                         gsr_rise_start_ta = current_ta
                                     gsr_peak_ta = max(gsr_peak_ta, current_ta)
                                 else:
                                     gsr_trend_rising = False
                                 
                                 # Detect drops
                                 is_flicker = d_ta < -GSR_FLICKER_THRESH
                                 is_strong_drop = d_ta < -GSR_RELIEF_THRESH
                                 
                                 # Calculate recovery percentage
                                 if gsr_peak_ta > gsr_rise_start_ta:
                                     rise_amount = gsr_peak_ta - gsr_rise_start_ta
                                     drop_from_peak = gsr_peak_ta - current_ta
                                 ...
                                 # ============ CONFLICT DETECTION ============
                                 # Flicker during rising trend, stays near peak, high Beta/Gamma
                                 if is_flicker and gsr_trend_rising and recovery_pct < GSR_RECOVERY_CONFLICT:
                                     if beta >= EEG_CONFLICT_BETA_MIN or gamma >= EEG_CONFLICT_GAMMA_MIN:
                                         biofeedback_state['conflict'] = True
                                         biofeedback_state['conflict_hold_until'] = current_time + 2.0  # Hold for 2s
                                 
                                 # ============ RELIEF DETECTION ============
                                 # Strong drop, high recovery, Delta elevated, Beta/Gamma low
                                 if is_strong_drop and recovery_pct >= GSR_RECOVERY_RELIEF:
                                     if (current_time - gsr_last_drop_time) > 5.0:  # Min 5s between relief events
                                         # EEG confirmation (either Delta high OR Beta/Gamma low)
                                         eeg_confirms = (delta >= EEG_RELIEF_DELTA_MIN or 
                                                        (beta <= EEG_RELIEF_BETA_MAX and gamma <= EEG_RELIEF_GAMMA_MAX))
                                         if eeg_confirms:
                                             biofeedback_state['relief'] = True
                                             biofeedback_state['relief_hold_until'] = current_time + 3.0  # Hold for 3s
                                             gsr_last_drop_time = current_time
                                             # Reset tracking after relief
                                             gsr_peak_ta = current_ta
                                             gsr_rise_start_ta = current_ta
                                 
                                 # Clear flags after hold time expires
                                 if current_time > biofeedback_state['conflict_hold_until']:
                                     biofeedback_state['conflict'] = False
                                 if current_time > biofeedback_state['relief_hold_until']:
                                     biofeedback_state['relief'] = False
                             
                             # [LEGACY] Keep surge_flags updated for backwards compatibility
                             SURGE_THRESHOLD = 1.3
                             MIN_POWER = 5.0
                             for k, val, idx in [('Delta', bands[0], 'delta'), ('Theta', bands[1], 'theta'), ('Gamma', bands[4], 'gamma')]:
                                 if len(running_bands[k]) > 10:
                                     avg = sum(running_bands[k]) / len(running_bands[k])
                                     if val > (avg * SURGE_THRESHOLD) and val > MIN_POWER:
                                         surge_flags[idx] = True
                                     else:
                                         surge_flags[idx] = False

                     
                     # No Secondary Smoothing (Band History) - As requested ("Most accurate measurement in this moment")
                     smoothed_bands = bands
                     
                     raw_focus = calculate_focus_score(bands)
                     raw_calm = int(bands[2]) # Alpha as Calm proxy
                     
                     # [FIX] RESTORE ROLLING SMOOTHING (Matched to v16/v20)
                     focus_history.append(raw_focus)
                     calm_history.append(raw_calm)
                     
                     if len(focus_history) > 0:
                         current_focus_score = int(sum(focus_history) / len(focus_history))
                     else: current_focus_score = raw_focus
                         
                     if len(calm_history) > 0:
                         current_calm_score = int(sum(calm_history) / len(calm_history))
                     else: current_calm_score = raw_calm
    
                # LOGGING: Prepare Batches
                if global_session_logger and global_session_logger.is_recording:
                     # [FIX] V10-Style Timestamping (Based on Sample Count, not Arrival Time)
                     # This ensures perfect monotonicity and no gaps even if BLE jitters.
                     base_samples = global_session_logger.recorded_samples
                     
                     # We are processing 'filt_chunk'. We need to know where we are in the chunk.
                     # BUT wait: 'global_session_logger.recorded_samples' is updated AFTER writerows.
                     # So we can use it as the "start index" for this batch?
                     # No, 'log_eeg_chunk' increments it.
                     # So we use the current value as base.
                     
                     # Wait! log_eeg_chunk is called AFTER this loop builds 'batch_rows'.
                     # So 'base_samples' is constant for this whole chunk loop?
                     # NO! 'log_eeg_chunk' is called once at the end.
                     # So the sample index for 'i' in this loop is 'base_samples + i'. ('i' is from enumerate(filt_chunk))
                     
                     # WAIT! We iterate 'filt_chunk' (Chunk M).
                     # 'i' goes 0..M.
                     # Sample Count is 'Total + i'.
                     
                     current_idx = base_samples + i
                     elapsed = current_idx / FS
                     sample_ts = global_session_logger.start_time + elapsed
                     
                     eeg_conn = 1 # Always 1 if we are here
                     eeg_on = 1 if headset_on_head else 0
                     
                     row = [
                        f"{sample_ts:.4f}", f"{elapsed:.4f}",
                        eeg_conn, eeg_on, current_focus_score, current_calm_score,
                        f"{smoothed_bands[0]:.2f}", f"{smoothed_bands[1]:.2f}", f"{smoothed_bands[2]:.2f}", 
                        f"{smoothed_bands[3]:.2f}", f"{smoothed_bands[4]:.2f}",
                        extracted[i]
                     ]
                     batch_rows.append(row)
            
            # [FIX] Flush Batch
            if batch_rows and global_session_logger:
                 global_session_logger.log_eeg_chunk(batch_rows)
        ...
        log_msg(f"[CRITICAL] Handler Error: {e}")
    ...
    while True:
        try:
            ...
            if not timestamp_answered and b'\x18\x00' in data and b'\x32\x04\x10' in data:
                ...
                log_msg("TIMESTAMP ANSWERED") # [DEBUG] Confirm logic runs
                timestamp_answered = True
        except Exception as e:
            log_msg(f"[LISTENER CRASH] {e}")
    ...
    while app_running:
        try:
            ...
            try:
                ...
                # [FIX] Reset Signal Timer on Connect to prevent instant "OFF" trigger
                last_on_signal_time = time.time()
                batt_timer = 0 # [FIX] Initialize Keepalive Timer
                ...
                while client.is_connected and app_running:
                     ...
                     # [NEW] Watchdog: DISABLED (Reverting to v10 logic to test stability)
                     # if (time.time() - last_packet_time) > 5.0:
                     #     log_msg("[BLE] Watchdog: No Data. Resetting...")
                     #     break # Breaks inner loop -> Disconnect -> Retry
                     
                     # [DEBUG] BLE Loop Heartbeat (every ~10s)
                     now_sec = int(time.time())
                     if now_sec % 10 == 0 and now_sec != globals().get('last_alive_sec', 0):
                         globals()['last_alive_sec'] = now_sec
                         if 'global_session_logger' in globals() and global_session_logger:
                             global_session_logger.log_system("BLE Loop Alive") # [FIX] Visible Diagnostic

                     # [FIX] Battery Polling DISABLED (Matches v10 exactly)
                     # User confirmed v10 works without polling. 
                     # Active polling was likely causing collisions/stalls.
                     # batt_timer += 1
                     # if batt_timer >= 200: # 200 * 0.1s = 20.0s
                     #     try:
                     ...
                     # [FIX] Match v10 loop speed (0.05s) for better responsiveness
                     await asyncio.sleep(0.05) 
...
class GSRReader(threading.Thread):
    ...
    def run(self):
        try:
            ...
            # [FIX] Relax polling to prevent GIL starvation of BLE Thread
            # HID devices typically update at 60Hz-125Hz. Polling at 500Hz (0.002s) is wasteful
            time.sleep(0.015) # ~66Hz

        except Exception as e:
            # print(f"[GSR] Error: {e}") # Suppress noise if device missing
            self.connected = False
...
class AudioRecorder:
    ...
    # [FIX] Make filename optional so manual stop doesn't crash
    def stop(self, filename=None):
        ...
        elif self.frames and not filename:
             print("[AUDIO] Frames buffered but no filename argument provided (Will save on Cleanup?)")
        else:
...
class ProMeterAppV28:
    def __init__(self, root):
        ...
        # [NEW] Bio-Feedback Indicators
        # Create a control frame for these indicators, positioned appropriately
        # For now, let's place it near the bottom status bar or similar.
        # This will be drawn on the canvas, not a separate Tkinter frame.
        # We'll define the indicators as part of the canvas drawing.
        ...
        # [NEW] Session Timer UI Stubs
        self.rec_start_time_str = "--:--:--"
        self.rec_duration_str = "00:00"
        ...
        self.sensor.start() 
        # print("[DEBUG] GSR THREAD DISABLED")
        
        self.load_gsr_config() # [NEW] Load settings on startup
        
        # [NEW] Bio-Feedback Indicators (Canvas elements)
        # ============================================================
        # EXPERIMENTAL: These indicators are based on preliminary 
        # analysis and are SUBJECTIVE INFERENCES. They should be used 
        # as exploratory tools, not definitive measures.
        # ============================================================
        def create_ind_canvas(x, y, text, color):
            # Wider box to fit full text
            self.canvas.create_rectangle(x - 55, y - 15, x + 55, y + 15, fill="#eee", outline="#999", tags="bio_indicator_bg")
            # Oval indicator (LED)
            oval = self.canvas.create_oval(x - 50, y - 10, x - 30, y + 10, fill="#ccc", outline="#999", tags="bio_indicator_oval")
            # Text label - shifted right to avoid overlapping LED
            self.canvas.create_text(x - 15, y, text=text, fill="#333", font=("Arial", 10, "bold"), anchor=tk.W, tags="bio_indicator_text")
            return oval, color

        # Position for bio-feedback indicators: ABOVE the graph, centered
        bio_x_start = WIN_W / 2 - 70
        bio_y = 400 # Above graph (GRAPH_Y starts at 500)

        self.ind_release_oval, self.ind_release_color = create_ind_canvas(bio_x_start, bio_y, "RELIEF", "#44aa44")
        self.ind_conflict_oval, self.ind_conflict_color = create_ind_canvas(bio_x_start + 140, bio_y, "CONFLICT", "#ff4400")
    ...
    def toggle_rec(self):
        if self.logger.is_recording:
            # STOP immediately
            self.logger.stop() 
            # [FIX] Pass filename to stop()
            wav_path = os.path.join(self.logger.folder_path, "audio.wav")
            self.audio_recorder.stop(filename=wav_path)
            self.save_session_notes() # [FIX] Explicitly save notes on Stop
    ...
    def save_session_notes(self):
        if self.session_notes:
             try:
                 if hasattr(self.logger, 'folder_path') and self.logger.folder_path:
                     txt_fn = os.path.join(self.logger.folder_path, "notes.txt")
                     with open(txt_fn, 'w') as f: f.write(self.session_notes)
                     print(f"[NOTES] Saved: {txt_fn}")
                     self.session_notes = "" # Reset
             except Exception as e:
    ...
    def start_session(self):
        ...
        if self.audio_recorder.is_recording and started_at:
            ...
            self.update_rec_button()
            self.session_timer = started_at
            
            # [NEW] Set Start Strings
            dt = datetime.datetime.fromtimestamp(started_at)
            self.rec_start_time_str = dt.strftime("%Y-%m-%d %H:%M:%S")
            self.rec_duration_str = "00:00"
        else:
    ...
    def update_gui(self):
        global current_state, is_connected, surge_flags # surge_flags needs to be global for this to work
        ...
        # [FIX] Do NOT delete bio indicators - they are persistent and only itemconfig updated
        
        # [NEW] Update Timer String
        if self.logger.is_recording and hasattr(self, 'session_timer'):
             elapsed = time.time() - self.session_timer
             m = int(elapsed // 60)
             s = int(elapsed % 60)
             self.rec_duration_str = f"{m:02}:{s:02}"
        ...
        if is_conn:
            ...
            # --- DUAL STREAM LOGGING ---
            # In v21, we ALWAYS log GSR from here to gsr.csv.
            # EEG thread logs to eeg.csv. No conflict.
            self.logger.log_gsr_sample(ta, self.center_ta, self.get_effective_zoom(), self.is_artifact)
            ...
            # [NEW] Session Timer (Center)
            cx = WIN_W / 2
            cy = WIN_H - 140
            # Start Time
            self.canvas.create_text(cx, cy - 25, text="START DATE & TIME", fill="#777", font=("Arial", 8, "bold"), tags="overlay")
            self.canvas.create_text(cx, cy - 10, text=self.rec_start_time_str, fill="#333", font=("Arial", 12, "bold"), tags="overlay")
            # Duration
            lbl_dur = "DURATION" if self.logger.is_recording else "LENGTH"
            self.canvas.create_text(cx, cy + 15, text=lbl_dur, fill="#777", font=("Arial", 8, "bold"), tags="overlay")
            self.canvas.create_text(cx, cy + 30, text=self.rec_duration_str, fill="#333", font=("Arial", 14, "bold"), tags="overlay")
        ...
        # [NEW] Update Bio-Feedback Indicators (Using Combined GSR+EEG Detection)
        try:
             # RELIEF: Combined GSR drop + EEG confirmation
             is_relief = biofeedback_state['relief']
             self.canvas.itemconfig(self.ind_release_oval, fill=self.ind_release_color if is_relief else "#ccc")
             
             # CONFLICT: GSR flicker during rising trend + high Beta/Gamma
             is_conflict = biofeedback_state['conflict']
             self.canvas.itemconfig(self.ind_conflict_oval, fill=self.ind_conflict_color if is_conflict else "#ccc")
             
             # Ensure indicators are on top
             self.canvas.tag_raise("bio_indicator_bg")
             self.canvas.tag_raise("bio_indicator_oval")
             self.canvas.tag_raise("bio_indicator_text")
        except: pass
        ...
        if self.logger.is_recording:
             ...
             if self.audio_recorder.is_recording:
                 ...
                 # Stop and Save
                 if hasattr(self.logger, 'folder_path') and self.logger.folder_path:
                      # [FIX] Use Session Folder
                      wav_fn = f"{self.logger.folder_path}/audio.wav"
                 elif hasattr(self.logger, 'filename') and self.logger.filename:
                 ...
                 if hasattr(self, 'settings_win') and self.settings_win and self.settings_win.winfo_exists():
                      ...
                      self.settings_win.start_monitor() 
                 
                 # Save Notes
                 if self.session_notes:
                     if hasattr(self.logger, 'folder_path') and self.logger.folder_path:
                         txt_fn = f"{self.logger.folder_path}/notes.txt"
                     elif 'fn' in locals() and fn:
